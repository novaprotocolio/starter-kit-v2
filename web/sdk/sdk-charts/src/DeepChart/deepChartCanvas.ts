...
export class OrderbookDeepChart extends BaseCanvas {
  ...
  private getRange = () => {
    ...
    const index =
      Math.ceil(
        this.zoom
          .times(bidCount > askCount ? bidCount : askCount)
  ...
  private drawXAxis() {
    ...
    for (let i = 0; i <= labelCount; i++) {
      ...
      // round(x) => precision(this.e + 1 +x)
      if (labelPrice.e !== null) {
        const labelPriceApproxi = labelPrice
          .precision(labelPrice.e + 1 + this.options.priceDecimals!, BigNumber.ROUND_HALF_EVEN)
          .toFixed(this.options.priceDecimals!);

        const labelText = this.options.formatXAxisLabel
          ? this.options.formatXAxisLabel(labelPriceApproxi)
          : labelPriceApproxi.toString();
  ...
  private getYByAmount(amount: BigNumber): BigNumber {
    ...
    return deepAreaHeight
      .minus(amount.div(this.maxAmount).times(deepAreaHeight))
      .plus((this.options.paddingTop! * this.ratio).toString());
  }
    ...
    if (this.bids[0] && price.lte(this.bids[0][0])) {
      ...
      for (let i = 0; i < this.bids.length; i++) {
        if (this.bids[i][0].gte(price)) {
          totalAmount = totalAmount.plus(this.bids[i][1]);
          totalCost = totalCost.plus(this.bids[i][1].times(this.bids[i][0]));
        } else {
          break;
        }
      }
      ...
      for (let i = 0; i < this.asks.length; i++) {
        if (this.asks[i][0].lte(price)) {
          totalAmount = totalAmount.plus(this.asks[i][1]);
          totalCost = totalCost.plus(this.asks[i][1].times(this.asks[i][0]));
        } else {
          break;
        }
      }
  ...
  private drawYAxis() {
    ...
    for (let i = 1; i < 5; i++) {
      ...
      const amount = this.maxAmount.minus(this.maxAmount.div(100).times(i * 25));
  ...
  private prepareData() {
    ...
    for (let i = 0; i < this.bids.length; i++) {
      ...
      bidAmount = bidAmount.plus(this.bids[i][1]);
    }
    ...
    for (let i = 0; i < this.asks.length; i++) {
      ...
      askAmount = askAmount.plus(this.asks[i][1]);
    }
    this.maxAmount = BigNumber.max(askAmount, bidAmount).times(1.5);
  }
