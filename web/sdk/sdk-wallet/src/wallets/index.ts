  ...
  return new Promise((resolve, reject) => {
    request.post(
      globalNodeUrl,
      {
        ...
        body: `{"jsonrpc":"2.0","method":"net_version","params":[],"id":${payloadId()}}`
      },
      (error, response, data) => {
        if (error) {
          reject(error);
...
export const sendRawTransaction = (data: string): Promise<string> => {
  return new Promise((resolve, reject) => {
    request.post(
      ...
      {
        headers: { "content-type": "application/json" },
        body: `{"jsonrpc":"2.0","method":"eth_sendRawTransaction","params":["${data}"],"id":${payloadId()}}`
      },
      (error, response, data) => {
        if (error) {
          reject(error);
...
export const getBalance = (address: string): Promise<BigNumber> => {
  return new Promise((resolve, reject) => {
    request.post(
      ...
      {
        headers: { "content-type": "application/json" },
        body: `{"jsonrpc":"2.0","method":"eth_getBalance","params":["${address}", "latest"],"id":${payloadId()}}`
      },
      (error, response, data) => {
        if (error) {
          reject(error);
...
export const getTransactionReceipt = (txID: string): Promise<any> => {
  return new Promise((resolve, reject) => {
    request.post(
      ...
      {
        headers: { "content-type": "application/json" },
        body: `{"jsonrpc":"2.0","method":"eth_getTransactionReceipt","params":["${txID}"],"id":${payloadId()}}`
      },
      (error, response, data) => {
        if (error) {
          reject(error);
...
export const getEstimateGas = (params: any): Promise<any> => {
  return new Promise((resolve, reject) => {
    request.post(
      ...
      {
        headers: { "content-type": "application/json" },
        body: `{"jsonrpc":"2.0","method":"eth_estimateGas","params":[${JSON.stringify(params)}],"id":${payloadId()}}`
      },
      (error, response, data) => {
        if (error) {
          reject(error);
...
export const getTransactionCount = (address: string, status: string = "latest"): Promise<number> => {
  return new Promise((resolve, reject) => {
    request.post(
      ...
      {
        headers: { "content-type": "application/json" },
        body: `{"jsonrpc":"2.0","method":"eth_getTransactionCount","params":["${address}","${status}"],"id":${payloadId()}}`
      },
      (error, response, data) => {
        if (error) {
          reject(error);
...
export const defaultWalletTypes = [ExtensionWallet.TYPE, NovaWallet.TYPE, WalletConnectWallet.TYPE, Ledger.TYPE];
